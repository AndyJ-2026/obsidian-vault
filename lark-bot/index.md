...
setup.sh            ← 一键安装脚本
start-local.sh      ← 启动脚本（操作 LaunchAgent，支持 restart/stop/status）
com.jaker.lark-bot.plist ← LaunchAgent 配置（开机自启+崩溃恢复）
...
```bash
cd ~/lark-bot && ./start-local.sh          # 重启
./start-local.sh stop                      # 停止
./start-local.sh status                    # 查看状态
```

通过 LaunchAgent 管理进程，开机自启 + 崩溃自动恢复 + 进程锁防多实例。
...
| 功能 | 群聊 @bot | 私聊（所有人） | 私聊（仅 owner） |
|------|----------|-------------|---------------|
| 建任务/约会议/查日程/查人/读文档/设提醒 | 所有人 | 所有人 | |
| 消息汇总/日报 | 所有人 | | 仅 owner |
| 会议转写 | | | 仅 owner |
...
- [[需求/稳定性与新能力/index|稳定性与新能力]] — 进程管理、断线重连、任务/日程/读文档
- [[需求/进程管理修复/index|进程管理修复]] — LaunchAgent 统一管理、进程锁
